...
class JobFilterManager:
    ...
    def _matches_filter_impl(self, title_lower: str) -> Tuple[bool, Tuple, float, Dict]:
        ...
        # Fast exclusion check (early return)
        for pattern in self.exclusion_compiled:
        ...
        # Optimized: Break early if we reach high confidence threshold
        for pattern, phrase in self.phrase_patterns:
            if pattern.search(title_lower):
                matched_keywords.append(phrase)
                
                # Get categories once, not repeated
                categories = self.keyword_to_category[phrase]
                phrase_weight_multiplier = 2.0
            ...
            # Early exit optimization: if score is very high, no need to continue
            if total_score >= 6.0:
                matched_categories = self._build_categories(matched_keywords)
                return True, tuple(matched_categories), total_score, {
                    'matched_keywords': matched_keywords,
                    'category_scores': category_scores,
                    'keyword_count': len(matched_keywords),
                    'match_type': 'high_confidence_phrase_match'
                }
        
        # Early exit if we have decent confidence match from phrases alone
        if total_score >= 4.0:
            matched_categories = self._build_categories(matched_keywords)
            return True, tuple(matched_categories), total_score, {
                'matched_keywords': matched_keywords,
        ...
        # Optimized: Only process if score still below threshold
        if total_score < 1.5:
            for pattern, keyword in self.single_patterns:
                if pattern.search(title_lower):
                    ...
                    for cat in categories:
                        ...
                        cat_name = cat['display_name']
                        category_scores[cat_name] = category_scores.get(cat_name, 0.0) + weight
        ...
        match_details = {
            ...
            'category_scores': category_scores,
            'keyword_count': len(matched_keywords),
            'match_type': 'combined_match' if len(matched_keywords) > 1 else ('single_match' if is_match else 'no_match')
        }
